...
void test_grid_inflation() {
  ...
  assert(map.is_occupied({10, 10}));
  assert(map.cost({11, 10}) > 0);
  assert(map.collides({map.cell_center({10, 10}), 0.0}, 0.25));
  assert(!map.collides({map.cell_center({2, 2}), 0.0}, 0.25));
}
...
void test_ekf_converges_with_measurements() {
  ...
  assert(near(ekf.pose().position.x, 1.0, 0.15));
  assert(near(ekf.pose().position.y, 0.0, 0.08));
  assert(near(ekf.pose().yaw, 0.0, 0.05));
  assert(near(ekf.velocity(), 1.0, 0.08));
  assert(ekf.covariance_trace() < 1.0);
}
...
void test_lattice_planner_selects_forward_motion() {
  ...
  assert(plan.success);
  assert(plan.command.linear > 0.1);
  assert(std::abs(plan.command.angular) < 0.5);
}
...
void test_mission_simulation_reaches_goal() {
  ...
  assert(report.final_error < 0.55);
  assert(report.replans > 0);
  assert(report.lidar_scans > 0);
  assert(report.covariance_trace < 0.5);
}
